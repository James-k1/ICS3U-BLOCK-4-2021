...
public class DoubleArraySequence {
   ...
   public void addAll(DoubleArraySequence addend) {
      if (addend==null){
         throw new NullPointerException("DoubleArraySequence Was Null.");
      }
      ensureCapacity(manyItems+addend.size());
      for (int i=manyItems, j=0; i<data.length;i++,j++){
   ...
   public static DoubleArraySequence catenation(DoubleArraySequence s1, DoubleArraySequence s2) {
      ...
      DoubleArraySequence newArr = new DoubleArraySequence(s1.size()+s2.size());
      newArr.addAll(s1);
      newArr.addAll(s2);
      newArr.setCurrentIndex(newArr.size());
      return newArr;
   }
